...
import { NeOverlayTunnelFormDialogComponent } from './overlay-tunnel-form-dialog.component';
import { NeOverlayTunnelDetailDialogComponent } from './overlay-tunnel-detail-dialog.component';

// Create/edit allowed, no delete - decommission via status (DOWN is the closest equivalent).
@Component({
  selector: 'app-ne-overlay-tunnels-list',
  standalone: true,
...
export class NeOverlayTunnelsListComponent implements OnInit {
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(NeOverlayTunnelFormDialogComponent, {
      width: '560px',
      data: null,
    });
  ...
  protected onView(row: OverlayTunnel): void {
    this.dialog.open(NeOverlayTunnelDetailDialogComponent, { width: '480px', data: row });
  }
