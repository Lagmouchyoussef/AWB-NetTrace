...
import { NeEquipmentTypeFormDialogComponent } from './equipment-type-form-dialog.component';
import { NeEquipmentTypeDetailDialogComponent } from './equipment-type-detail-dialog.component';

// Create/edit allowed, no delete - decommission via status (DEPRECATED).
@Component({
  selector: 'app-ne-equipment-types-list',
  standalone: true,
...
export class NeEquipmentTypesListComponent implements OnInit {
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(NeEquipmentTypeFormDialogComponent, {
      width: '560px',
      data: null,
    });
  ...
  protected onView(row: EquipmentType): void {
    this.dialog.open(NeEquipmentTypeDetailDialogComponent, { width: '480px', data: row });
  }
