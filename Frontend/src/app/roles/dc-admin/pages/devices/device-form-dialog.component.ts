...
@Component({
  selector: 'app-dc-admin-device-form-dialog',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    MatDialogModule,
...
export class DcAdminDeviceFormDialogComponent implements OnInit {
  private readonly deviceService = inject(DcAdminDeviceService);
  private readonly rackService = inject(DcAdminRackService);
  private readonly dialogRef = inject(MatDialogRef<DcAdminDeviceFormDialogComponent, boolean>);
  ...
  protected readonly form = new FormGroup({
    ...
    deviceType: new FormControl<DeviceType>(this.data?.deviceType ?? 'SERVER', {
      nonNullable: true,
      validators: [Validators.required],
    }),
