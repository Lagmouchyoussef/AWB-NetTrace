...
import { DcAdminCableFormDialogComponent } from './cable-form-dialog.component';
import { DcAdminCableDetailDialogComponent } from './cable-detail-dialog.component';

@Component({
  selector: 'app-dc-admin-cables-list',
  standalone: true,
...
export class DcAdminCablesListComponent implements OnInit {
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(DcAdminCableFormDialogComponent, { width: '560px', data: null });
  ...
  protected onView(row: Cable): void {
    this.dialog.open(DcAdminCableDetailDialogComponent, { width: '480px', data: row });
  }

  protected onEdit(row: Cable): void {
    const ref = this.dialog.open(DcAdminCableFormDialogComponent, { width: '560px', data: row });
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.load();
      }
  ...
  protected async onDelete(row: Cable): Promise<void> {
    const confirmData: ConfirmDialogData = {
      titleKey: 'cables.deleteTitle',
      messageKey: 'cables.deleteMessage',
    ...
    if (confirmed) {
      await this.cableService.delete(row.id);
      this.load();
    }
  }
