...
import { DcAdminRackFormDialogComponent } from './rack-form-dialog.component';
import { DcAdminRackDetailDialogComponent } from './rack-detail-dialog.component';

@Component({
  selector: 'app-dc-admin-racks-list',
  standalone: true,
...
export class DcAdminRacksListComponent implements OnInit {
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(DcAdminRackFormDialogComponent, { width: '560px', data: null });
  ...
  protected onView(row: Rack): void {
    this.dialog.open(DcAdminRackDetailDialogComponent, { width: '480px', data: row });
  }

  protected onEdit(row: Rack): void {
    const ref = this.dialog.open(DcAdminRackFormDialogComponent, { width: '560px', data: row });
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.load();
      }
