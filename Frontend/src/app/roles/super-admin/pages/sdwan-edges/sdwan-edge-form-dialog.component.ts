...
import { SDWAN_EDGE_STATUSES, SdwanEdge, SdwanEdgeStatus } from './sdwan-edge.model';

@Component({
  selector: 'app-sdwan-edge-form-dialog',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    MatDialogModule,
...
export class SdwanEdgeFormDialogComponent implements OnInit {
  private readonly sdwanEdgeService = inject(SdwanEdgeService);
  private readonly datacenterService = inject(DatacenterService);
  private readonly dialogRef = inject(MatDialogRef<SdwanEdgeFormDialogComponent, boolean>);
