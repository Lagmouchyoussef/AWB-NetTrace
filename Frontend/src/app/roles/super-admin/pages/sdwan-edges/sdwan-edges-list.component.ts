...
import { SdwanEdgeFormDialogComponent } from './sdwan-edge-form-dialog.component';
import { SdwanEdgeDetailDialogComponent } from './sdwan-edge-detail-dialog.component';

@Component({
  selector: 'app-sdwan-edges-list',
  standalone: true,
...
export class SdwanEdgesListComponent implements OnInit {
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(SdwanEdgeFormDialogComponent, { width: '560px', data: null });
  ...
  protected onView(row: SdwanEdge): void {
    this.dialog.open(SdwanEdgeDetailDialogComponent, { width: '480px', data: row });
  }

  protected onEdit(row: SdwanEdge): void {
    const ref = this.dialog.open(SdwanEdgeFormDialogComponent, { width: '560px', data: row });
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.load();
      }
