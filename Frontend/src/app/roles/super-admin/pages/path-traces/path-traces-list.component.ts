...
import { PathTraceService } from '../../../../core/services/path-trace.service';
import { downloadCsv } from '../../../../core/utils/csv-export';
import { PathTrace } from './path-trace.model';
import { PathTraceFormDialogComponent } from './path-trace-form-dialog.component';
import { PathTraceDetailDialogComponent } from './path-trace-detail-dialog.component';

@Component({
  selector: 'app-path-traces-list',
  standalone: true,
...
export class PathTracesListComponent implements OnInit {
  private readonly pathTraceService = inject(PathTraceService);
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(PathTraceFormDialogComponent, { width: '560px', data: null });
  ...
  protected onView(row: PathTrace): void {
    this.dialog.open(PathTraceDetailDialogComponent, { width: '480px', data: row });
  }

  protected onEdit(row: PathTrace): void {
    const ref = this.dialog.open(PathTraceFormDialogComponent, { width: '560px', data: row });
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.load();
      }
