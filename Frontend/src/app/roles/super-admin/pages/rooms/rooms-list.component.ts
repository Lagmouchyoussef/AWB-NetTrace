...
export class RoomsListComponent implements OnInit {
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(RoomFormDialogComponent, { width: '560px', data: null });
  ...
  protected onView(row: Room): void {
    this.dialog.open(RoomDetailDialogComponent, { width: '480px', data: row });
  }

  protected onEdit(row: Room): void {
    const ref = this.dialog.open(RoomFormDialogComponent, { width: '560px', data: row });
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.load();
      }
