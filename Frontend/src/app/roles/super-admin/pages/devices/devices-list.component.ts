...
export class DevicesListComponent implements OnInit {
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(DeviceFormDialogComponent, { width: '560px', data: null });
  ...
  protected onView(row: Device): void {
    this.dialog.open(DeviceDetailDialogComponent, { width: '480px', data: row });
  }

  protected onEdit(row: Device): void {
    const ref = this.dialog.open(DeviceFormDialogComponent, { width: '560px', data: row });
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.load();
      }
