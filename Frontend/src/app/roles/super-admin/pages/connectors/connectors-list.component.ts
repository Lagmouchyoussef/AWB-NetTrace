...
import { ConnectorDetailDialogComponent } from './connector-detail-dialog.component';
...
export class ConnectorsListComponent implements OnInit {
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(ConnectorFormDialogComponent, { width: '560px', data: null });
  ...
  protected onView(row: Connector): void {
    this.dialog.open(ConnectorDetailDialogComponent, { width: '480px', data: row });
  }

  protected onEdit(row: Connector): void {
    const ref = this.dialog.open(ConnectorFormDialogComponent, { width: '560px', data: row });
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.load();
      }
