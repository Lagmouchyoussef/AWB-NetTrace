...
import { OverlayTunnelService } from '../../../../core/services/overlay-tunnel.service';
import { downloadCsv } from '../../../../core/utils/csv-export';
import { OverlayTunnel } from './overlay-tunnel.model';
import { OverlayTunnelFormDialogComponent } from './overlay-tunnel-form-dialog.component';
import { OverlayTunnelDetailDialogComponent } from './overlay-tunnel-detail-dialog.component';

@Component({
  selector: 'app-overlay-tunnels-list',
  standalone: true,
...
export class OverlayTunnelsListComponent implements OnInit {
  private readonly overlayTunnelService = inject(OverlayTunnelService);
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(OverlayTunnelFormDialogComponent, { width: '560px', data: null });
  ...
  protected onView(row: OverlayTunnel): void {
    this.dialog.open(OverlayTunnelDetailDialogComponent, { width: '480px', data: row });
  }

  protected onEdit(row: OverlayTunnel): void {
    const ref = this.dialog.open(OverlayTunnelFormDialogComponent, { width: '560px', data: row });
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.load();
      }
