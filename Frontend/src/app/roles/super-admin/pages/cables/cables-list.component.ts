...
import { CableFormDialogComponent } from './cable-form-dialog.component';
import { CableDetailDialogComponent } from './cable-detail-dialog.component';
...
export class CablesListComponent implements OnInit {
  private readonly cableService = inject(CableService);
  ...
  protected onCreate(): void {
    const ref = this.dialog.open(CableFormDialogComponent, { width: '560px', data: null });
  ...
  protected onView(row: Cable): void {
    this.dialog.open(CableDetailDialogComponent, { width: '480px', data: row });
  }

  protected onEdit(row: Cable): void {
    const ref = this.dialog.open(CableFormDialogComponent, { width: '560px', data: row });
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.load();
      }
